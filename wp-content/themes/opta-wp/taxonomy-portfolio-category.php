...
<ul class="grid" id="portfolio">          
    ...
    <?php while (have_posts()) : the_post(); ?>
        ...
        if (has_post_thumbnail($post->ID)) {
            ...
            $portfolio_post_thumb = '<img src = "' . esc_url(get_template_directory_uri()) . '/images/no-photo.png" alt = "" />';
        }
        ...
        if ($item_quote !== '') {
            ...
            switch ($link_thumb_to):
                case 'link_to_this_post':
                    $portfolio_content.= '<li class="grid-item element-item animate"><div class="item-wrapper"><a href="' . get_the_permalink($post->ID) . '">' . $portfolio_post_thumb . '</a><div class="portfolio-text-holder"><div class="portfolio-info"><a class="portfolio-text" href="' . get_the_permalink($post->ID) . '">' . $p_thumb_text . '</a>' . opta_get_cat($post->ID) . '<p class="portfolio-arrow"><a href="' . get_the_permalink($post->ID) . '"><img src="' . esc_url(get_template_directory_uri()) . '/images/opta_arrow@2x.png" alt="" /></a></p></div></div></div></li>';
                    break;
                case 'link_to_extern_url':
                    $extern_site_url = get_post_meta($post->ID, "portfolio_extern_site_url", true);
                    $portfolio_content.= '<li class="grid-item element-item animate"><div class="item-wrapper"><a href="' . $extern_site_url . '" target="_blank">' . $portfolio_post_thumb . '</a><div class="portfolio-text-holder"><div class="portfolio-info"><a class="portfolio-text" href="' . $extern_site_url . '" target="_blank">' . $p_thumb_text . '</a>' . opta_get_cat($post->ID) . '<p class="portfolio-arrow"><a href="' . $extern_site_url . '" target="_blank"><img src="' . esc_url(get_template_directory_uri()) . '/images/opta_arrow@2x.png" alt="" /></a></p></div></div></div></li>';
                    break;
                case 'no_link':
