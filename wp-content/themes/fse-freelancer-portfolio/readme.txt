=== FSE Freelancer Portfolio ===
Contributors: cretathemes
Requires at least: 5.9
Tested up to: 6.4
Requires PHP: 5.6
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: blog, portfolio, photography, one-column, two-columns, left-sidebar, right-sidebar, block-patterns, block-styles, custom-background, custom-logo, custom-menu, featured-images, footer-widgets, full-site-editing, threaded-comments, wide-blocks, translation-ready

== Description ==

FSE Freelancer Portfolio is a visually appealing and feature-rich WordPress theme designed specifically for freelancers, creative professionals, resumes, designers, web developers,  and individuals looking to showcase their work and services online. With a modern and elegant design, FSE Freelancer Portfolio offers a user-friendly experience, enabling users to effortlessly create a captivating online portfolio. The theme provides a range of customizable blocks and layouts that are tailored to present various types of content, such as project galleries, testimonials, blog posts, and service offerings. It offers an intuitive drag-and-drop interface for building and editing content. Key features of the FSE Freelancer Portfolio theme include responsive design, ensuring that websites look and function well across different devices and screen sizes. The theme also supports high-quality media presentation, enabling users to display their visual work, such as images and videos, in a polished and impactful manner. FSE Freelancer Portfolio emphasises user engagement by offering easy navigation, clear calls-to-action, and integration with social media platforms. This helps freelancers and creative professionals connect with potential clients and audiences effectively. The theme’s focus on elegant design, customization options, and content showcase capabilities make it a valuable tool for freelancers to effectively display their skills, attract clients, and leave a lasting impression on their website visitors.

== Changelog ==

= 1.0.0 (Released: Aug 19, 2023) =
* Initial Release

= 1.0.1 (Released: Aug 23, 2023) =
* Changed © 2023 FSE Freelancer Portfolio.
* Changed email123.com.
* Deleted index.php file.

= 1.0.2 (Released: Sep 02, 2023) =
* Added get-started.
* Added urls.
* Added footer link.

= 1.0.3 (Released: Sep 18, 2023) =
* Phone number change.
* submenu color change.
* Submenu hover color change.
* Added comment section css on post and pages.
* change previous page button color.
* change next page button color.
* Added customize.

= 1.0.4 (Released: Oct 13, 2023) =
* Updated 404 page.
* Updated sidebar.
* Resolved responsive media.
* Added gallery images.
* Added sidebar in single page.
* Updated search page.
* Added text-domain in all templates file.

= 1.0.5 (Released: Oct 18, 2023) =
* Bugs Resolved

= 1.0.6 (Released: Oct 19, 2023) =
* Responsive resolved.

= 1.0.7 (Released: Nov 06, 2023) =
* Updated index page.
* Added sidebar in index page.
* Added archive in sidebar.
* Added single page sidebar on left.
* Added single post sidebar on left.

= 1.0.8 (Released: Nov 22, 2023) =
* Added page with no heading/feature image.
* Removed redirection function.
* Updated tested up to version.

= 1.0.9 (Released: Dec 07, 2023) =
* Added wow animations.
* Added animation css file.
* Added wow js file.

= 1.1 (Released: Dec 21, 2023) =
* Added sidebar on search page.
* Added recent post in footer.

= 1.1.1 (Released: Jan 10, 2023) =
* Added animation on footer.
* Added archive product file.
* Added cart file.
* Added checkout file.
* Added single product file.
* Added woocommerce css.
* Updated archive header file.

= 1.1.2 (Released: Jan 22, 2023) =
* Added noto sans font family.
* Added Grape Nuts font family.
* Added Lamon font family.
* Added montserrat font family.
* Updated theme.json file.

= 1.1.3 (Released: Feb 7, 2023) =
* Updated search.html file.
* Updated single.html file.
* Adedd notice function.
* Removed index.php file.
* Added single post no sidebar.

= 1.1.4 (Released: Mar 4, 2024) =
* Removed wow.js file.
* Updated classes for animation.
* Added animation.min.css file.
* Removed wow js licensed.

== Copyright ==

FSE Freelancer Portfolio WordPress Theme, 2012-2023
FSE Freelancer Portfolio is distributed under the terms of the GNU GPL.

FSE Freelancer Portfolio is based on Catch Bells WordPress Theme, 2012-2023
Catch Bells is distributed under the terms of the GNU GPL.

== Resources ==

= WOW JS =
* Matthieu Aussaguel
* Copyright 2016, Matthieu Aussaguel
* https://github.com/matthieua/WOW
* License: GPLv3
* https://github.com/matthieua/WOW/blob/master/README.md

= Animate CSS =
* Daniel Eden
* Copyright 2021 Daniel Eden
* https://github.com/animate-css/animate.css
* License: 2.1
* https://github.com/animate-css/animate.css/blob/main/LICENSE

Google Web Fonts ( Poppins ) By Google - https://google.com
* Poppins, This is the normal family, Licensed under Open Font License, https://fonts.google.com/specimen/Poppins?query=poppins

Google Web Fonts ( Noto Sans ) By Google - https://google.com
* Noto Sans, This is the normal family, Licensed under Open Font License, https://fonts.google.com/noto/specimen/Noto+Sans?query=Noto+Sans

Google Web Fonts ( Grape Nuts ) By Google - https://google.com
* Grape Nuts, This is the normal family, Licensed under Open Font License, https://fonts.google.com/specimen/Grape+Nuts?query=Grape+Nuts

Google Web Fonts ( Lemon ) By Google - https://google.com
* Lemon, This is the normal family, Licensed under Open Font License, https://fonts.google.com/specimen/Lemon?query=Lemon

Google Web Fonts ( Montserrat ) By Google - https://google.com
* Montserrat, This is the normal family, Licensed under Open Font License, https://fonts.google.com/specimen/Montserrat?query=Montserrat

* Images

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1573331

//Gallery

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1450067

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1092332

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1447187

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1433619

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/753019

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/759001