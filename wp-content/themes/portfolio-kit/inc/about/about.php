 ...
 *
 */
function portfolio_kit_pnotice_output()
{
?>
	<div class="mgadin-hero">
		<div class="mge-info-content">
			<div class="mge-info-hello">
				...
				$demo_link = esc_url('https://pkit.wpteamx.com/portfolio-kit-demo/');
				$pro_link = esc_url('https://wpthemespace.com/product/portfolio-kit-pro/?add-to-cart=7230');

				esc_html_e('Hello, ', 'portfolio-kit');
				echo esc_html($current_user->display_name);
				?>

				<?php esc_html_e('👋🏻', 'portfolio-kit'); ?>
			</div>
			<div class="mge-info-desc">
				<div><?php printf(esc_html__('Attention, don\'t miss out Pro version! 🚨 Now You are using the Free version of Portfolio Kit theme.  Portfolio Kit PRO version is now available with an auto-setup feature!! Enjoy SEO-friendly and lightweight design, lightning-fast speed optimization, more secure, premade demos, effortless one-click demo imports, and exclusive custom Elementor premium widgets. With the pro version, you can take your website to the next level and truly stand out from the competition.', 'portfolio-kit'), $current_theme_name); ?></div>
				<div class="mge-offer"><?php printf(esc_html__('Don\'t miss out on these incredible features - upgrade to Portfolio Kit Pro now! ', 'portfolio-kit'), $current_theme_name); ?></div>
			</div>
			<div class="mge-info-actions">
				<a href="<?php echo esc_url($pro_link); ?>" target="_blank" class="button button-primary upgrade-btn">
					<?php esc_html_e('Upgrade Pro', 'portfolio-kit'); ?>
				</a>
				<a href="<?php echo esc_url($demo_link); ?>" target="_blank" class="button button-primary demo-btn">
					<?php esc_html_e('View Details', 'portfolio-kit'); ?>
				</a>
				<button class="button button-info btnend"><?php esc_html_e('Dismiss this notice', 'portfolio-kit') ?></button>
			</div>
...
{
	$hide_date = get_option('portfoliokit_info_date1');
	if (!empty($hide_date)) {
...
?>
	<div class="mgadin-notice notice notice-info mgadin-theme-dashboard mgadin-theme-dashboard-notice mge is-dismissible meis-dismissible">
		<?php portfolio_kit_pnotice_output(); ?>
	</div>
<?php
...
function portfolio_kit_new_optins_texts_init()
{
	if (isset($_GET['xbnotice']) && $_GET['xbnotice'] == 1) {
		update_option('portfoliokit_info_date1', current_time('mysql'));
	}
}
